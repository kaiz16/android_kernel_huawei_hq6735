...
/* changed by jeffrey to align Android behavior */
				...
				/* DBGLOG(SW4, INFO,("SWCTRL_RX_FILTER: g_u4RXFilter %x ucOpt0 %x ucOpt1 %x fgUpdate %x u4rxfilter %x, rStatus %x\n", */
		...
		case SWCTRL_RX_ARP_OFFLOAD:
			{
				...
				if (ucOpt1 == 1) {
					if (wlanGetIPV4Address(prAdapter->prGlueInfo, pucIp, &u4NumIPv4) && u4NumIPv4 > 3) {	/* TODO: repleace 3 to macro */
						u4NumIPv4 = 3;
					}
				} else if (ucOpt1 == 0) {
					u4NumIPv4 = u4NumIPv6 = 0;
				}
				printk("u4Len:%d bufSize:%d u4NumIPv4:%d\n", u4Len, bufSize,
				       u4NumIPv4);
				...
				for (i = 0; i < u4NumIPv4; i++) {
					...
					prParamIpAddr =
					    (P_PARAM_NETWORK_ADDRESS_IP) prParamNetAddr->aucAddress;
...
#if 0
#ifdef CONFIG_IPV6
				if (!wlanGetIPV6Address(prAdapter->prGlueInfo,
							pucIp,
							&u4NumIPv6)
				...
				for (i = 0; i < u4NumIPv6; i++) {
					...
					prParamNetAddr->u2AddressType = PARAM_PROTOCOL_ID_TCP_IP;
					kalMemCopy(prParamNetAddr->aucAddress, pucIpv6 + (i * 16),
						   16);
					prParamNetAddr =
					    (P_PARAM_NETWORK_ADDRESS) ((UINT_32) prParamNetAddr +
								       sizeof(ip6));
...
testPsSendQoSNullFrame(IN P_ADAPTER_T prAdapter,
		       ...
		       IN BOOLEAN ucPacketType, IN BOOLEAN ucPsSessionID, IN BOOLEAN fgSetEOSP)
{
	...
	TX_SET_MMPDU(prAdapter,
		     ...
		     ucBssIndex,
		     prStaRec->ucIndex,
		     WLAN_MAC_HEADER_QOS_LEN, WLAN_MAC_HEADER_QOS_LEN, NULL, MSDU_RATE_MODE_AUTO);
	...
	if (prCmdSwCtrl) {
		if (prCmdSwCtrl->u4Data == SWCR_DBG_TYPE_ALL) {
			...
			DBGLOG(SW4, INFO, ("TX0\n"
					   ...
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_TX_AGING_TIMEOUT_CNT],
			...
			DBGLOG(SW4, INFO, ("RX0\n"
					   ...
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_RX_CLASS_ERROR_DROP_CNT],
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_RX_AMPDU_ERROR_DROP_CNT],
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_RX_STATUS_ERROR_DROP_CNT],
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_RX_FORMAT_ERROR_DROP_CNT],
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_RX_ICV_ERROR_DROP_CNT],
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_RX_KEY_ERROR_DROP_CNT],
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_RX_TKIP_ERROR_DROP_CNT],
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_RX_MIC_ERROR_DROP_CNT],
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_RX_BIP_ERROR_DROP_CNT],
			...
			DBGLOG(SW4, INFO, ("PWR\n"
					   ...
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_PWR_TRIGGER_NULL_CNT],
					   prCmdSwCtrl->u4DebugCnt[SWCR_DBG_ALL_PWR_BCN_IND_CNT],
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_PWR_BCN_TIMEOUT_CNT],
			...
			DBGLOG(SW4, INFO, ("ARM\n"
					   ...
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_AR_STA0_RX_RATE_RCPI],
			...
			DBGLOG(SW4, INFO, ("BB\n"
					   ...
					   prCmdSwCtrl->
					   u4DebugCnt[SWCR_DBG_ALL_BB_OFDM_SIGERR_CNT]));
