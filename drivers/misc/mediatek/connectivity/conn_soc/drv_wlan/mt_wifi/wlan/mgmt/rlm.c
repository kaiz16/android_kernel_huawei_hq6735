...
#if (CFG_SUPPORT_BCN_TUNE == 1)
/* ----------------------------------------------------- */
/* Purpose: customize beacon timeout detection parameters */
/* ------------------------------------------------------*/
VOID
rlmBcnEnhanceConfig(
	P_ADAPTER_T         prAdapter
	)
{
	...
	kalMemZero(&rBcnInfo, sizeof(CMD_RLM_BCN_INFO_T));
	/*	u4Version: current is 0
		u4BcnTT_NumOfContLostThreshold: number of continuous lost beacon, 
			if reach this threshold, then report beacon timeout event to driver.
			valid valude should be 10 ~ 40. the detect time should be
			u4BcnTT_NumOfContLostThreshold * TBTT, usually TBTT=100TU (0.1s)
		u4BcnTT_fgKeepAwakeInLastStage: wait a TBTT to enter power save;
			otherwise, enter power save immediately.
	*/
	rBcnInfo.u4Version = 0;
	rBcnInfo.u4BcnTT_NumOfContLostThreshold = 20;
	rBcnInfo.u4BcnTT_fgKeepAwakeInLastStage = 1;

	DBGLOG(INIT, INFO, ("Config beacon related parameters\n"));
	...
	{
		DBGLOG(INIT, WARN, ("rlmBcnEnhanceConfig fail 0x%lx\n", rStatus));
	}
}
#endif
