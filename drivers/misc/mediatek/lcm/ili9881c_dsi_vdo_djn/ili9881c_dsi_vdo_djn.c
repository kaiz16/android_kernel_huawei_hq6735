...
{0x53,1,{0x72}},             //VCOM1
{0x55,1,{0x77}},               //VCOM2
{0x50,1,{0x75}},               // VREG1OUT=4.7V
{0x51,1,{0x71}},               // VREG2OUT=-4.7V
{0x60,1,{0x1b}},               //SDT
{0x61,1,{0x01}},
{0x62,1,{0x0c}},
{0x63,1,{0x00}},
...
static void lcm_init(void)
{
	...
	if(ret) 	
		printf("[LK]ili9881c_dji ----tps6132----ret=%0x--i2c write error----\n",ret);		
	else
		printf("[LK]ili9881c_dji ----tps6132----ret=%0x--i2c write success----\n",ret);			
#else
	ret=tps65132_write_bytes(0x0, 0xC);

	if(ret<0)
		printk("[KERNEL]ili9881c_dji ----tps6132---ret=%0x-- i2c write error-----\n",ret);
	else
		printk("[KERNEL]ili9881c_dji ----tps6132---ret=%0x-- i2c write success-----\n",ret);
#endif
	MDELAY(5);
	...
	if(ret) 	
		dprintf(0, "[LK]ili9881c_dji ----tps6132----ret=%0x--i2c write error----\n",ret);		
	else
		dprintf(0, "[LK]ili9881c_dji ----tps6132----ret=%0x--i2c write success----\n",ret);	 
#else
...
static unsigned int lcm_compare_id(void)
{
	...
	ret=TPS65132_write_byte(0x0 ,0xC); //5.2V
	if(ret) 	
		dprintf("[LK]ili9881c_dsi_vdo_djn ----tps65132----ret=%0x--i2c write error----\n",ret);		
	else
		dprintf("[LK]ili9881c_dsi_vdo_djn ----tps6132----ret=%0x--i2c write success----\n",ret);			
#else
	ret=tps65132_write_bytes(0x0, 0xC);

	if(ret<0)
		printk("[KERNEL]ili9881c_dsi_vdo_djn ----tps65132---ret=%0x-- i2c write error-----\n",ret);
	else
		printk("[KERNEL]ili9881c_dsi_vdo_djn ----tps65132---ret=%0x-- i2c write success-----\n",ret);
#endif
	MDELAY(5);
	mt_set_gpio_mode(GPIO_LCD_ENN, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_LCD_ENN, GPIO_DIR_OUT);
	...
	ret=TPS65132_write_byte(0x1, 0xC); //-5.2v
	if(ret) 	
		dprintf(0, "[LK]ili9881c_dsi_vdo_djn ----tps65132----ret=%0x--i2c write error----\n",ret);		
	else
		dprintf(0, "[LK]ili9881c_dsi_vdo_djn ----tps65132----ret=%0x--i2c write success----\n",ret);  
#else
	ret=tps65132_write_bytes(0x1, 0xC);
	if(ret<0)
		printk("[KERNEL]ili9881c_dsi_vdo_djn ----tps6132---ret=%0x-- i2c write error-----\n",ret);
	else
		printk("[KERNEL]ili9881c_dsi_vdo_djn ----tps6132---ret=%0x-- i2c write success-----\n",ret);
#endif
	MDELAY(5);
	...
	id = buffer[0];
		...
		printf("jinmin ili9881c_dsi_vdo_djn [lk]=%d %d %d \n", buffer[0],buffer[1], buffer[2]);
		printf("id =0x%x\n", id);
#else
		printk("jinmin ili9881c_dsi_vdo_djn [kernel]=%d %d %d \n", buffer[0],buffer[1], buffer[2]);
		printk("id =0x%x\n", id);
#endif
