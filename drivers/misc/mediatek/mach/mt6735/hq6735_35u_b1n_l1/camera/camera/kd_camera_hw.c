...
u32 pinSet[3][8] = {
    ...
    if (On) {
...
#endif
        if ((pinSetIdx == 0) && ((currSensorName && (0 != strcmp(SENSOR_DRVNAME_OV13850QT_MIPI_RAW, currSensorName)) 
	   	&& (0 != strcmp(SENSOR_DRVNAME_OV13850_MIPI_RAW, currSensorName))))){
        ...
        {   
             PK_DBG("SENSOR_DRVNAME_OV13850_MIPI_RAW poweron \n");
             ISP_MCLK1_EN(1);
             mdelay(2);
		...
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV13850QT_MIPI_RAW, currSensorName)))
        {   
        
             PK_DBG("SENSOR_DRVNAME_OV13850QT_MIPI_RAW poweron \n");
             ISP_MCLK1_EN(1);
             mdelay(2);
		...
		else if ((currSensorName && (0 == strcmp(currSensorName,"hi843b808mipiraw"))))
        {
				//First Power Pin low and Reset Pin Low
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]) {
        ...
        else  if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP2508_RAW, currSensorName)))
        {
       ...
       if ((pinSetIdx == 0) && ((currSensorName && (0 != strcmp(SENSOR_DRVNAME_OV13850QT_MIPI_RAW, currSensorName)) 
        ...
        {
            //Set Power Pin low and Reset Pin Low
		        PK_DBG("SENSOR_DRVNAME_OV13850_MIPI_RAW off ");
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! (CMRST)\n");}
                if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMRST],GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! (CMRST)\n");}
                if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_OFF])){PK_DBG("[CAMERA SENSOR] set gpio failed!! (CMRST)\n");}
