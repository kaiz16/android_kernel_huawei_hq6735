...
//#define OV13850QT_R2A_write_i2c(addr, para) iWriteReg((u16) addr , (u32) para , 1, OV13850QTMIPI_WRITE_ID)
#define PFX "OV13850QT_R2A_OTP"
#define LOG_INF(format, args...)        xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " format, __FUNCTION__, ##args)

#define Delay(ms)  mdelay(ms)
static unsigned char OV13850QTMIPI_WRITE_ID = 0X00;

kal_uint16 OV13850QT_R2A_read_i2c(kal_uint32 addr)
{
    kal_uint16 get_byte=0;
    iReadReg((u16) addr ,(u8*)&get_byte,OV13850QTMIPI_WRITE_ID);
    return get_byte;
}

kal_uint16 OV13850QT_R2A_write_i2c(addr, para)
{
		iWriteReg((u16) addr , (u32) para , 1, OV13850QTMIPI_WRITE_ID);
		return 1;
}
#endif
#if 0
void otp_cali(unsigned char writeid)
{
	...
	OV13850QTMIPI_WRITE_ID = writeid;
...
int read_otp(struct otp_struct *otp_ptr,unsigned char writeid)
{
	LOG_INF(" liyongjian >>>>start read otp\n");
	OV13850QTMIPI_WRITE_ID = writeid;
	int otp_flag=0;
	int addr=0;
	int temp=0;
	...
	//set 0x5002[1] to "0"
	int temp1=0;
	temp1 = OV13850QT_R2A_read_i2c(0x5002);
	OV13850QT_R2A_write_i2c(0x5002, (0x00 & 0x02) | (temp1 & (~0x02)));
	// read OTP into buffer
	OV13850QT_R2A_write_i2c(0x3d84, 0xC0);
	OV13850QT_R2A_write_i2c(0x3d88, 0x72); // OTP start address
	OV13850QT_R2A_write_i2c(0x3d89, 0x20);
	OV13850QT_R2A_write_i2c(0x3d8A, 0x73); // OTP end address
	OV13850QT_R2A_write_i2c(0x3d8B, 0xBE);
	OV13850QT_R2A_write_i2c(0x3d81, 0x01); // load otp into buffer
	Delay(10);
	// OTP base information and WB calibration data
	otp_flag = OV13850QT_R2A_read_i2c(0x7220);
	LOG_INF(" liyongjian >>>> WB calibration data : %x \n", otp_flag);
	addr = 0;
	...
	if(addr != 0) {
		...
		(*otp_ptr).module_integrator_id = OV13850QT_R2A_read_i2c(addr);
		(*otp_ptr).lens_id = OV13850QT_R2A_read_i2c( addr + 1);
		(*otp_ptr).production_year = OV13850QT_R2A_read_i2c( addr + 2);
		(*otp_ptr).production_month = OV13850QT_R2A_read_i2c( addr + 3);
		(*otp_ptr).production_day = OV13850QT_R2A_read_i2c(addr + 4);
		temp = OV13850QT_R2A_read_i2c(addr + 7);
		(*otp_ptr).rg_ratio = (OV13850QT_R2A_read_i2c(addr + 5)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (OV13850QT_R2A_read_i2c(addr + 6)<<2) + ((temp>>4) & 0x03);	
	}
	else {
	...
	LOG_INF(" liyongjian >>>> module date in otp ,year = %d,month = %d,day = %d \n", (*otp_ptr).production_year,(*otp_ptr).production_month,(*otp_ptr).production_day);
	LOG_INF("liyongjian>>>> AWB data : (*otp_ptr).rg_ratio = %d, (*otp_ptr).bg_ratio = %d\n", (*otp_ptr).rg_ratio,(*otp_ptr).bg_ratio);
	// OTP Lenc Calibration
	otp_flag = OV13850QT_R2A_read_i2c(0x7231);
	LOG_INF("liyongjian>>>> Lenc calibration data : %x \n", otp_flag);
	addr = 0;
...
if(addr != 0) {
		for(i=0;i<186;i++) {
			(* otp_ptr).lenc[i]= OV13850QT_R2A_read_i2c(addr + i);
			checksumLSC += (* otp_ptr).lenc[i];
			LOG_INF(" liyongjian >>> Lenc (* otp_ptr).lenc[%d] : %x \n", i, (* otp_ptr).lenc[i]);
		}
		...
		(* otp_ptr).checksumLSC=OV13850QT_R2A_read_i2c(addr + 186);
		(* otp_ptr).checksumOTP=OV13850QT_R2A_read_i2c(addr + 187);
		(* otp_ptr).checksumTotal=OV13850QT_R2A_read_i2c(addr + 188);
	...
	{
		OV13850QT_R2A_write_i2c(i,0); // clear OTP buffer, recommended use continuous write to accelarate
	}

	//set 0x5002[1] to "1"
	temp1 = OV13850QT_R2A_read_i2c(0x5002);
	OV13850QT_R2A_write_i2c(0x5002, (0x02 & 0x02) | (temp1 & (~0x02)));
	LOG_INF(" liyongjian >>>>end read otp\n");
	return (*otp_ptr).flag;
}
...
int apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ((*otp_ptr).flag & 0x40) {
		LOG_INF(" liyongjian>>>apply OTP WB Calibration : %x \n", (*otp_ptr).flag);
		rg = (*otp_ptr).rg_ratio;
		bg = (*otp_ptr).bg_ratio;
		...
		if (R_gain>0x400) {
			OV13850QT_R2A_write_i2c(0x5056, R_gain>>8);
			OV13850QT_R2A_write_i2c(0x5057, R_gain & 0x00ff);
		}

		if (G_gain>0x400) {
			OV13850QT_R2A_write_i2c(0x5058, G_gain>>8);
			OV13850QT_R2A_write_i2c(0x5059, G_gain & 0x00ff);
		}

		if (B_gain>0x400) {
			OV13850QT_R2A_write_i2c(0x505A, B_gain>>8);
			OV13850QT_R2A_write_i2c(0x505B, B_gain & 0x00ff);
		}
		LOG_INF(" liyongjian >>>>apply awb otp : R_gain = %x,B_gain = %x,G_gain = %x\n",R_gain,B_gain,G_gain);
	}
	
	// apply OTP Lenc Calibration
	if ((*otp_ptr).flag & 0x10) {
		
		LOG_INF(" liyongjian>>>>>apply OTP Lenc Calibration : %x \n", (*otp_ptr).flag);
		temp = OV13850QT_R2A_read_i2c(0x5000);
		temp = 0x01 | temp;
		OV13850QT_R2A_write_i2c(0x5000, temp);
		...
		if(Decode_13850R2A((*otp_ptr).lenc,  lenc_out))
		{
			for(i=0;i<360 ;i++) {
				...
				OV13850QT_R2A_write_i2c(0x5200 + i, lenc_out[i]);
			}
		}
	}
}
