...
static imgsensor_info_struct imgsensor_info = {
    ...
    .pre = {
        .pclk = 32500000,              //record different mode's pclk //21
...
static void sensor_init(void)
{
	...
	if(fromsd == 1)//是否从SD读取//gepeiwei   120903
      ...
      write_cmos_sensor(0xfb,0x73);//9.29canshu 63
			...
			write_cmos_sensor(0xfd,0x01);
      write_cmos_sensor(0x9d,0x96);//96//ea,20160711//d6 fs
			write_cmos_sensor(0xa0,0x05);//05//29,20160711
			write_cmos_sensor(0xa1,0x03);//02
			write_cmos_sensor(0xad,0x62);//
			write_cmos_sensor(0xae,0x00);//
			write_cmos_sensor(0xaf,0x85);//
			write_cmos_sensor(0xb1,0x01);
			//write_cmos_sensor(0xac,0x01);put in mode function
    	write_cmos_sensor(0xfd,0x01);
        ...
        do {
            *sensor_id = return_sensor_id();
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {
				...
				printk("gpw i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("gpw Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			printk("gpw Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
