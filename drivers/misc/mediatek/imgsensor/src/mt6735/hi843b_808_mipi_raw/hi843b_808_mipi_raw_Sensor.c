...
#include "hi843b_808_mipi_raw_Sensor.h"

#define PFX "HI843B_808_camera_sensor"
//#define LOG_WRN(format, args...) xlog_printk(ANDROID_LOG_WARN ,PFX, "[%S] " format, __FUNCTION__, ##args)
//#defineLOG_INF(format, args...) xlog_printk(ANDROID_LOG_INFO ,PFX, "[%s] " format, __FUNCTION__, ##args)
//#define LOG_DBG(format, args...) xlog_printk(ANDROID_LOG_DEBUG ,PFX, "[%S] " format, __FUNCTION__, ##args)
#define LOG_INF(format, args...) xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " format, __FUNCTION__, ##args)
#define Hi843B_808_MaxGain 16
#define Hi843B_808_I2C_BURST // If  open, you can use "i2c burst mode". 
#define HI843B_808_OTP_FUNCTION


static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = HI843B_808_SENSOR_ID,
	
	.checksum_value = 0xa340b362,
	
	.pre = {
		.pclk = 288000000,				//record different mode's pclk
		.linelength = 3800, //3800,				//record different mode's linelength
		.framelength = 2520, //2520,			//record different mode's framelength
		.startx =0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width = 3264, //3264,		//record different mode's width of grabwindow
		.grabwindow_height = 2448, //2448,		//record different mode's height of grabwindow
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    if (gain < BASEGAIN || gain > Hi843B_808_MaxGain * BASEGAIN) {
        LOG_INF("Error gain setting");

        if (gain < BASEGAIN)
            gain = BASEGAIN;
        else if (gain > Hi843B_808_MaxGain * BASEGAIN)
            gain = Hi843B_808_MaxGain * BASEGAIN;
    }
...
#ifdef HI843B_808_OTP_FUNCTION
static struct HI843B_otp_struct 
{
...
static struct HI843B_otp_struct HI843B_otp;


static void HI843B_OTPSetting(void)
{	
	LOG_INF("HI843B_808 OTPSetting begin:\n ");
	...
	LOG_INF("HI843B_808 OTPSetting exit :\n ");
}
...
LOG_INF("HI843B addr = 0x%x \n", addr);
	if (addr == 0) {
		...
		HI843B_otp.IR_id = OTP_read_cmos_sensor(addr + 10);
		HI843B_otp.infocheck = OTP_read_cmos_sensor(addr + 16);
	}

	checksum = (HI843B_otp.module_integrator_id + HI843B_otp.AF_Flag + HI843B_otp.prodyction_year + HI843B_otp.production_month + HI843B_otp.production_day + HI843B_otp.sensor_id + HI843B_otp.lens_id + HI843B_otp.vcm_id + HI843B_otp.Driver_ic_id + HI843B_otp.F_num_id + HI843B_otp.IR_id) % 0x100;
...
//WB info
	...
	if (HI843B_otp.WB_FLAG == 0x01)
		...
		wb_start_addr = 0x0C70;
	else if (HI843B_otp.WB_FLAG == 0x37)
		wb_start_addr = 0x0C80;
	else
...
//AF info
	...
	if(af_atart_addr != 0) 
   {
     ...
     HI843B_otp.afcheck = HI843B_otp.af_data[6];
   }
   else 
   {
	...
	checksum = (HI843B_otp.af_data[0] + HI843B_otp.af_data[1] + HI843B_otp.af_data[2] + HI843B_otp.af_data[3]) % 0x100;
...
static int HI843B_otp_apply(void)
{
	...
	wbcheck = HI843B_otp.wb_data[15];
	//checksum = (HI843B_otp.wb_data[0] + HI843B_otp.wb_data[1] + HI843B_otp.wb_data[2] + HI843B_otp.wb_data[3] + HI843B_otp.wb_data[4] + HI843B_otp.wb_data[5] + HI843B_otp.wb_data[6] + HI843B_otp.wb_data[7] + HI843B_otp.wb_data[8] + HI843B_otp.wb_data[9] + HI843B_otp.wb_data[10] + HI843B_otp.wb_data[11] ) % 0x100;
	for(i=0;i<15;i++)
	{
	checksum += HI843B_otp.wb_data[i];
	}
...
write_cmos_sensor(0x0b00, 0x0018); //Terry modified for mipi test 20170103
...
write_cmos_sensor(0x0b22, 0xeb80); //cc80); //Terry modified for mipi test 20170103
write_cmos_sensor(0x0b24, 0xa690); //Terry modified for mipi test 20170103
...
write_cmos_sensor(0x0a04, 0x017b); 
...
write_cmos_sensor(0x0b1e, 0x0081); 
write_cmos_sensor(0x0b20, 0x0800); 
write_cmos_sensor(0x0b22, 0xeb80); //Terry modified for mipi test 20170103
write_cmos_sensor(0x0b24, 0xa690); //Terry modified for mipi test 20170103
write_cmos_sensor(0x0b26, 0x0001); 
write_cmos_sensor(0x0b28, 0x0807); 
...
write_cmos_sensor(0x0002, 0x0000); 
write_cmos_sensor(0x0a02, 0x0100); 
write_cmos_sensor(0x0a04, 0x015b); 
...
write_cmos_sensor(0x000c, 0x0022); 
write_cmos_sensor(0x0012, 0x0162);
...
write_cmos_sensor(0x000c, 0x0022); 
write_cmos_sensor(0x0012, 0x0168); //0x016c); 
write_cmos_sensor(0x0018, 0x0b77); //0x0b73); 
write_cmos_sensor(0x001e, 0x1111); 
write_cmos_sensor(0x0008, 0x0ed8); 
write_cmos_sensor(0x0034, 0x0700); 
write_cmos_sensor(0x0022, 0x0008); 
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			do {
				*sensor_id =return_sensor_id();
				printk("travies check sensor id is 0x%x,i2c write id is 0x%x\n",*sensor_id,imgsensor.i2c_write_id);
				(*sensor_id) ++;/*module id for hi843b_808 is 0x0844,travies*/
...
UINT32 HI843B_808_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	printk("HI843B_808_MIPI_RAW_SensorInit");
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	HI843B_808_MIPI_RAW_SensorInit	*/
