...
#define OV8858_OTP_SUPPORT


#ifdef OV8858_OTP_SUPPORT
...
static kal_uint32 open(void)
{
	...
	//read_otp(&otp_ptr);
	printk("zjy ov8858-sunny-otp valid! modid: %d\n", otp_ptr.module_integrator_id);
	//apply_otp(&otp_ptr);
	printk("zjy ov8858-sunny-otp valid! lens_id: %d, year:%d\n", otp_ptr.lens_id, otp_ptr.production_year);
#endif
	spin_lock(&imgsensor_drv_lock);
