...
kal_int32 g_Q_MAX_POS_50[] = {Q_MAX_POS_50,Q_MAX_POS_f_50,Q_MAX_POS_g_50};
kal_int32 g_Q_MAX_POS_25[] = {Q_MAX_POS_25, Q_MAX_POS_f_25, Q_MAX_POS_g_25};
kal_int32 g_Q_MAX_POS_0[] = {Q_MAX_POS_0, Q_MAX_POS_f_0,Q_MAX_POS_g_0};
kal_int32 g_Q_MAX_NEG_10[] = {Q_MAX_NEG_10, Q_MAX_NEG_f_10,Q_MAX_NEG_g_10};
kal_int32 g_Q_MAX_POS_50_H_CURRENT[] = {Q_MAX_POS_50_H_CURRENT, Q_MAX_POS_f_50_H_CURRENT,Q_MAX_POS_g_50_H_CURRENT};
kal_int32 g_Q_MAX_POS_25_H_CURRENT[] = {Q_MAX_POS_25_H_CURRENT, Q_MAX_POS_f_25_H_CURRENT,Q_MAX_POS_g_25_H_CURRENT};
kal_int32 g_Q_MAX_POS_0_H_CURRENT[] = {Q_MAX_POS_0_H_CURRENT, Q_MAX_POS_f_0_H_CURRENT,Q_MAX_POS_g_0_H_CURRENT};
kal_int32 g_Q_MAX_NEG_10_H_CURRENT[] = {Q_MAX_NEG_10_H_CURRENT, Q_MAX_NEG_f_10_H_CURRENT,Q_MAX_NEG_g_10_H_CURRENT};
...
//add begin by wangyang 2015 12 23 
int get_Ubvbat(void) 
{
	char *ptr;
	int ubvbat1;
...
//add end by wangyang 2015 12 23
...
/*
	...
	else if ((400000 > id_volt)&&(id_volt > 200000))
		g_fg_battery_id = BATTERY_GY;
...
void dod_init(void)
{
	...
	gFG_voltage_save = gFG_voltage;
//add begin by wangyang 2015012 22 
	if(bat_is_charger_exist() == KAL_TRUE){//wangguan mark 0415 for battery
		int ubvbat=0;
		ubvbat=get_Ubvbat();
		bm_print(BM_LOG_CRTI, "[FGADC] TEST ubvbat=%d，gFG_voltage=%d\n", ubvbat,gFG_voltage);
		if(ubvbat){
			gFG_voltage=ubvbat;
		 }
		bm_print(BM_LOG_CRTI, "[FGADC] TEST ubvbat=%d\n", ubvbat);
	}
//wangguan mark 0415 for battery
//add end 
	...
	if ((abs(gFG_capacity_by_v_init - gFG_capacity_by_v) > 5)
	    ...
	    if (gFG_voltage_init<4000) {
	    	gFG_capacity_by_v_init = fgauge_read_capacity_by_v(gFG_voltage_init);
...
*/
		{
			...
			if (((g_rtc_fg_soc != 0)
				 ...
				 ((gFG_capacity_by_v > CUST_POWERON_LOW_CAPACITY_TOLRANCE
				   || bat_is_charger_exist() == KAL_TRUE)))
				   || ((g_rtc_fg_soc != 0)
				&& (g_boot_reason == BR_WDT_BY_PASS_PWK || g_boot_reason == BR_TOOL_BY_PASS_PWK))
				)
#endif
			{
				gFG_capacity_by_v = g_rtc_fg_soc;
...
/* 5. Logging */
	...
	printk("hw ocv gFG_voltage_save=%d\n",gFG_voltage_save);
...
kal_int32 battery_meter_initial(void)
{
	...
	//printk("[%s] meter_initilized:%d", __func__, meter_initilized);
